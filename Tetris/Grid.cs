...
namespace Tetris
{
	...
	class Grid
	{
		private List<int> idxFullRows = new List<int>();

		private PieceType[,] grid;

		private PieceType[,] returnArray;
        ...
        public void PlaceGhostPiece(Piece piece)
        {
            ...
            for (int i = 0; i < pieceArray.GetLength(0); i++)
            {
                ...
                for (int j = 0; j < pieceArray.GetLength(1); j++)
                {
                    ...
                    if (NotOutOfRange(piece.position.Y + ghostPiecePos + i, piece.position.X + j) && pieceArray[i, j] == piece.type)
                    {
