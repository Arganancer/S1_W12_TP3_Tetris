...
namespace Tetris
{
	...
	static class StaticVars
	{
		...
		static StaticVars()
		{
			...
			blockTextures = new Texture[] {		new Texture("Art/blocks/placeholder_I.png"),
			...
			imageSize = new Vector2i((int)StaticVars.blockTextures[0].Size.X, (int)StaticVars.blockTextures[0].Size.Y);

			gridXPos = Application.WINDOW_WIDTH / 2 - drawGrid.GetLength(1) * imageSize.X / 2;//(int)drawGrid[0,0].Size.X / 2;
			gridYPos = Application.WINDOW_HEIGHT / 2 - drawGrid.GetLength(0) * imageSize.Y / 2;//* (int)drawGrid[0, 0].Size.Y / 2;
			...
			controlsText = new Text("←/→ - Move piece\n↑ - Rotate piece\n↓ - Soft Drop\nSpace - Hard Drop\nC - Hold piece\nP - Pause\nEsc/M - Menu", font, 20);
