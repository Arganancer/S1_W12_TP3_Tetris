...
namespace TetrisDotnet.Code.Scenes
{
	public class GameScene : Scene
	{
		...
		private RealTimeText realTimeText = new RealTimeText();
		private StatsTextBlock statsTextBlock = new StatsTextBlock();
		private readonly ControlsText controlsText = new ControlsText();
			...
			if (gameState == GameState.Playing || gameState == GameState.Pause)
			{
				...
				for (int x = 0; x < Grid.GridWidth; x++)
				{
					for (int y = 0; y < Grid.GridHeight - 2; y++)
					{
						...
						PieceType block = grid.GetBlock(x, y);
