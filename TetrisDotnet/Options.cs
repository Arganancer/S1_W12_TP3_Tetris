...
namespace Tetris
{
	class Options
	{
		...
		public Options()
		{
			...
			menuItems = new string[][] {	new string[] {"Music A", "Music B", "Music C"},
											new string[] {"Off"},
											new string[] {"Change grid size"},
											new string[] {"Back"}};
			...
			for(int i = 0; i < menuItems.Length; i++)
			{
				...
				for (int j = 0; j < menuItems[i].Length; j++)
				{
					...
					text.Position = new Vector2f(	Application.WINDOW_WIDTH / 2 - ((menuItems[i].Length - 1) / 2 * (charSizeBuffer / 1.8f * longestWordsLens[i])) + (charSizeBuffer / 1.8f * longestWordsLens[i]) * j, 
													Application.WINDOW_HEIGHT / 2 - ((menuItems.Length * 0.5f) / 2 * charSizeBuffer) + (i * charSizeBuffer));
		...
		private void SetCursor()
		{
			...
			elementsToDraw[flatArrayLength + cursorPosX].DisplayedString = ">" + elementsToDraw[flatArrayLength + cursorPosX].DisplayedString;
		...
		private void RemoveCursor()
		{
			...
			elementsToDraw[flatArrayLength + cursorPosX].DisplayedString = elementsToDraw[flatArrayLength + cursorPosX].DisplayedString.Substring(1);
